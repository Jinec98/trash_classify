...
def upload():
    if request.method == 'POST':
        ...
        file_name = os.path.join(upload_path, secure_filename(filedata.filename))  # 指定保存文件夹的路径

        try:
            # 上传文件
            filedata.save(file_name)
            print(base_path, file_name, upload_path)
            #-------------------后端数据在这里!------------------
            listtest = ('1','10','30')
            datalist = sort(listtest)
            return render_template('Trash_selected_ok.html', fp=datalist[0], possible=datalist[1], name=datalist[2],
                                   catalog=datalist[3], same_catalog=datalist[4], fpf=upload_path)
        except IOError:
...
def sort(datalist):
    ...
    list = [fp, possible, name, catalog, same_catalog]
    return list
